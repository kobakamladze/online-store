...
import { useGetSingleDeviceQuery } from "../store/slices/apiSlice";
import { useAddToCartMutation } from "../store/slices/authApiSlice";
...
const DevicePage = () => {
  ...
  const { data, isLoading } = useGetSingleDeviceQuery(deviceId);
    ...
    <animated.div style={mountStyle}>
      <Container className="mt-4 d-flex justify-content-between">
        ...
        <Col md={6} className="d-flex flex-column p-2">
          ...
          <Button
            onClick={e => handleAddToCart(e, { userId: user.id, deviceId: id })}
          >
            Add to cart
          </Button>
        </Col>
